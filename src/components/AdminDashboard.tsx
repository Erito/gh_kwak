    ...
    return (
        ...
        >
            <motion.div
                ...
                className="bg-slate-50 rounded-2xl shadow-2xl w-full max-w-7xl h-[95vh] md:h-[90vh] flex flex-col relative overflow-hidden"
                onClick={(e) => e.stopPropagation()}
            >
                <header className="p-4 border-b border-slate-200 flex justify-between items-center shrink-0 bg-white">
                    <h2 className="text-lg md:text-xl font-bold text-slate-800">Admin Dashboard</h2>
                ...
                <div className="grow overflow-y-auto">
                    {sortedReports.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-slate-500 p-8">
                            ...
                            <p className="font-medium text-lg text-center">Belum ada data laporan</p>
                        </div>
                    ) : (
                        <div className="w-full overflow-x-auto">
                            ...
                            <div className="md:hidden divide-y divide-slate-100 bg-white">
                                {sortedReports.map((report, idx) => (
                                    <div key={report.ID_Laporan || idx} className="p-4 flex flex-col gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-slate-800">{report.Deskripsi}</p>
                                            {report.Instansi_Pelaksanaan && (
                                                <p className="text-[10px] text-slate-500 bg-green-50 p-1.5 rounded inline-block mt-1 border border-green-100">
                                                    <span className="font-bold text-green-700">Dikerjakan oleh: </span>{report.Instansi_Pelaksanaan}
                                                </p>
                                        ...
                                        <div className="mt-1">
                                            {String(report.Status).toUpperCase() !== "RESOLVED" ? (
                                                <button
                                                    onClick={() => report.ID_Laporan && setResolvingId(report.ID_Laporan)}
                                                    disabled={loading}
                                                    className="w-full text-center font-bold text-xs bg-green-100 text-green-700 border border-green-300 hover:bg-green-200 py-2.5 rounded-lg transition-colors disabled:opacity-50 shadow-sm"
                                                >
                                                    LAPOR SELESAI
                                                </button>
                                            ) : (
                                                <span className="w-full text-center text-xs text-green-500 font-bold flex items-center justify-center gap-1 bg-green-50 py-2 rounded-lg border border-green-100">
                                                    Tuntas ✓
                                                </span>
                                            )}
                                        </div>
